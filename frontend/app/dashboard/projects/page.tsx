    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '18px 36px 0', flexShrink: 0, borderBottom: '1px solid hsl(var(--dash-border-subtle))' }}>
        ...
        <div style={{ display: 'flex', gap: 16, marginBottom: 12, flexWrap: 'wrap' }}>
          ...
          <span style={{ fontSize: 11, letterSpacing: '-0.01em', color: 'hsl(var(--dash-fg-dim))' }}>
            Last edited: {formatDateTime(doc.updated_at)}
          </span>
        </div>
      </div>
...
export default function ProjectDocsPage() {
  ...
  const DOCS_LEFT_MIN = 220
  const DOCS_LEFT_MAX = 520
  const DOCS_LEFT_WIDTH_KEY = 'dashboard.docs.leftWidth'
  const DOCS_LEFT_COLLAPSED_KEY = 'dashboard.docs.leftCollapsed'
  ...
  useEffect(() => {
    const savedWidth = window.localStorage.getItem(DOCS_LEFT_WIDTH_KEY)
    const savedCollapsed = window.localStorage.getItem(DOCS_LEFT_COLLAPSED_KEY)
    const parsed = Number(savedWidth)
    if (!Number.isNaN(parsed) && parsed >= DOCS_LEFT_MIN && parsed <= DOCS_LEFT_MAX) {
      setLeftWidth(parsed)
    }
  ...
  useEffect(() => {
    window.localStorage.setItem(DOCS_LEFT_WIDTH_KEY, String(leftWidth))
  }, [leftWidth])

  useEffect(() => {
    window.localStorage.setItem(DOCS_LEFT_COLLAPSED_KEY, leftCollapsed ? '1' : '0')
  }, [leftCollapsed])

  useEffect(() => {
    if (!resizing) return
    const onMove = (e: MouseEvent) => {
      const next = Math.max(DOCS_LEFT_MIN, Math.min(DOCS_LEFT_MAX, e.clientX))
      setLeftWidth(next)
    }
    const onUp = () => {
      setResizing(false)
      ...
      <div style={{
        ...
        <div style={{ padding: '14px 14px 12px', borderBottom: '1px solid hsl(var(--dash-border-subtle))', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 11 }}>
            <span style={{ fontSize: 12, letterSpacing: '-0.01em', color: 'hsl(var(--dash-fg-dim))' }}>Docs</span>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <button
                onClick={() => setLeftCollapsed(true)}
                title="Minimize docs panel"
                style={{
                  display: 'flex',
                  alignItems: 'center',
          ...
          {filtered.map(doc => {
            ...
            return (
              ...
              >
                ...
                <div style={{ fontSize: 9, color: 'hsl(var(--dash-fg-dim))', textAlign: 'left' }}>
        ...
        >
          <div
            style={{
              ...
              width: 1,
              background: 'hsl(var(--dash-border-subtle))',
            }}
          />
        </div>
      )}
